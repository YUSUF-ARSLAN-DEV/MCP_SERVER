...
def explore(page, url: str) -> PageInventory:
    ...
    controls = page.locator('a,button,input,select,textarea,video,audio,[role]').evaluate_all("""els => els.filter(e => { const s=getComputedStyle(e); return s.display!=='none' && s.visibility!=='hidden' && e.getClientRects().length; }).slice(0,150).map(e => {
