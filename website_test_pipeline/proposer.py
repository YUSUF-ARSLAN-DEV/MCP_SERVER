...
RULES = (
    ...
    "across pages). Each flow is 2-%d steps and ends in an observable outcome.\n"
    "Rules:\n"
...
def _check_step(step: dict, index: dict, pages: set[str]) -> tuple[dict | None, str]:
    ...
    return {"page": page, "kind": _ACTIONS[action], "selector": control.get("selector"),
            "name": (control.get("name") or str(step.get("target")))[:40], "value": value}, ""


def validate_flow(raw: dict, index: dict, pages: set[str]) -> tuple[list[dict] | None, dict | None, str]:
    ...
    if effect == "navigates":
        ...
        if to not in pages:
            return None, None, f"outcome page {to} was not explored"
        outcome["to"] = to
    return steps, outcome, ""


def _signature(steps: list[dict]) -> tuple:
    return tuple((s.get("page"), s.get("kind"), _norm(s.get("name") or s.get("selector") or "")) for s in steps)
...
def propose(raw_flows: list[dict], inventories: list[dict], existing: list[dict],
    ...
    for raw in raw_flows[:MAX_FLOWS]:
        ...
        steps, outcome, reason = validate_flow(raw, index, pages)
        if steps is None:
            rejected.append((goal, reason))
            continue
        sig = _signature(steps)
        if sig in seen:
            rejected.append((goal, "duplicate of an existing flow"))
            continue
        seen.add(sig)
