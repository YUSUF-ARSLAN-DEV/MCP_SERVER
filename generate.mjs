...
async function callModel(prompt){
 if(!GEMINI_API_KEY) throw new Error('GEMINI_API_KEY is required for Gemini AI Studio');
 const body={contents:[{role:'user',parts:[{text:`You are a test spec generator. Return exactly one complete TypeScript code block and no prose.\n\n${prompt}`}]}],generationConfig:{temperature:0.1,maxOutputTokens:3072,responseMimeType:'text/plain'}};
 for(let attempt=0;attempt<=MODEL_RETRIES;attempt++){const id=++requestNumber,started=Date.now(),controller=new AbortController(),timer=setTimeout(()=>controller.abort(),MODEL_TIMEOUT_MS); try{
  const resp=await fetch(GEMINI_API_URL,{method:'POST',headers:{'Content-Type':'application/json','x-goog-api-key':GEMINI_API_KEY},body:JSON.stringify(body),signal:controller.signal}); const raw=await resp.text(); debug(`request #${id}`,`provider=gemini model=${GEMINI_MODEL} attempt=${attempt+1} status=${resp.status} ms=${Date.now()-started}`);
  if(!resp.ok){const e=new Error(`Gemini API error: ${resp.status}`);e.status=resp.status;e.body=raw.slice(0,1000);if(!isTransientStatus(resp.status)||attempt===MODEL_RETRIES)throw e;const ra=Number(resp.headers.get('retry-after'));await sleep(Number.isFinite(ra)?ra*1000:RETRY_BASE_MS*2**attempt);continue;}
  let data;try{data=JSON.parse(raw)}catch{const e=new Error('Gemini returned non-JSON success response');e.transient=true;e.body=raw.slice(0,1000);throw e;}
  if(data.error)throw new Error(`Gemini model error: ${JSON.stringify(data.error).slice(0,500)}`);
  const candidate=data.candidates?.[0]; const content=candidate?.content?.parts?.map(part=>part.text||'').join('');
  if(typeof content!=='string'||!content.trim())throw new Error(`Gemini response had no usable content (finishReason=${candidate?.finishReason||'unknown'})`);
  if(candidate.finishReason==='MAX_TOKENS')throw new Error('Gemini output was truncated'); return content;
