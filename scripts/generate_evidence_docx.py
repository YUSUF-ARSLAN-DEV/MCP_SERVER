...
def main(src, dest):
    data = json.load(open(src, encoding='utf-8')); results = data.get('results', []); doc = Document(); sec = doc.sections[0]; sec.top_margin = sec.bottom_margin = Inches(.65); sec.left_margin = sec.right_margin = Inches(.7); doc.styles['Normal'].font.name = 'Aptos'; doc.styles['Normal'].font.size = Pt(10)
    ...
    for i,x in enumerate(results,1):
        ...
        for a in x.get('attachments',[]):
            if os.path.exists(a['path']): doc.add_picture(a['path'],width=Inches(6.4)); cap=doc.add_paragraph(clean_text(a['name'])); cap.alignment=WD_ALIGN_PARAGRAPH.CENTER
